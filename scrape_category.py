...
def find_next_page(soup, list_url_product, url):
    ...
    next_page_soup = soup
    while next_page_soup.find("li", class_="next"):
        next_page = next_page_soup.find("li", class_="next").find("a").get("href")
        next_page_url = url[:-10] + next_page
        next_page_soup = scrape_product.scrape_a_page(next_page_url)
...
# Fait appel à la fonction scrape_product.main pour chaque produit et créé une liste avec les données de tous les produits de la catégorie
def scrape_all_products(list_url_product, url):
    list_data = []
    for link in list_url_product:
        list_data.append(scrape_product.main(link, url))
    return list_data
